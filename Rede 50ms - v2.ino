...
typedef struct __attribute__((packed)) {
    ...
    uint8_t intensidadeVibracao;   // 0x00 a 0xFF (0 a 255) - Potência do PWM de vibração (1 byte)
    uint16_t duracao;              // Duração da vibração em milissegundos (0 a 65.535 ms) (2 bytes)
    bool ladoDireito;              // true (1) = Aciona lado Direito | false (0) (1 byte)
    bool ladoEsquerdo;             // true (1) = Aciona lado Esquerdo | false (0) (1 byte)
    uint8_t pastaAudio;            // Número da pasta (ex: 0x01 a 0x63 / 1 a 99) (1 byte)
    uint16_t arquivoAudio;         // Identificador do arquivo/faixa (0x0000 a 0xFFFF / 0 a 65.535) (2 bytes)
} struct_message;
...
// Variáveis exclusivas do Transmissor
  ...
  const unsigned long TIMEOUT_ACK = 300;  // Tempo para chegada das confirmações
...
void OnDataRecv(const uint8_t *mac, const uint8_t *incomingData, int len) {
    ...
    if (len == sizeof(struct_message)) {
      ...
      Serial.printf("Pacote %d/5 recebido -> Vibracao: 0x%02X (%d/255), Duracao: %d ms, Audio: %d (Pasta: 0x%02X, Faixa: 0x%04X [%d])\n",
...
void loop() {
    ...
    if (millis() - tempoUltimoComando > 20 || tempoUltimoComando == 0) {
      ...
      data.audioAtivo = 1;
      data.intensidadeVibracao = 0xC8; // Ex: 200 em decimal
      ...
      data.arquivoAudio = 0x012C;      // Exemplo de faixa > 255: 300 em decimal (0x012C em hex)
